...
public class player_script : MonoBehaviour {
    ...
    void Movement()
    {
        ...
        float velocityX = 3 * motionDirection.x / motionDirection.y; ;       //uncomment the above line and comment this line before the game is finished
    ...
    void CalculateDestination()
    {
        ...
        destination.y = transform.position.y + Mathf.Abs((motionDirection.y / motionDirection.x) * (distanceFromOriginToWallCollider - (float)0.5 + Mathf.Abs(transform.position.x)-transform.localScale.x/2));
        //in the above equation, we have to make sure that the radius of the ball is 0.5 units
    ...
    void UpdateDestination()
    {
        ...
        destination.y -= 3 * Time.deltaTime;                                //same instruction as the comment in Movement()
    }


    //if the restricting angles are to be changed, change angles in the conditional statements - (20 and 70)
    //but the change should be symmetric around x=y line
    //i.e 45-325=20 and 45+25=70
    //else, the code needs to be changed
    Vector2 InputRestriction(Vector2 input)
    {
        if (Mathf.Atan2(input.y, input.x) < ((90 * leftOrRight + 20) * Mathf.Deg2Rad))
        {
            Vector2 returningVector = new Vector2(1, Mathf.Tan((90 * leftOrRight + 20) * Mathf.Deg2Rad));
            return returningVector;
        }
        else if (Mathf.Atan2(input.y, input.x) > ((90 * leftOrRight + 70) * Mathf.Deg2Rad))
        {
            Vector2 returningVector = new Vector2(1, Mathf.Tan((90 * leftOrRight + 70) * Mathf.Deg2Rad));
            return returningVector;
        }
        else
            return input;
